...
$("#sys_user_list").datagrid({
		title:'账号列表',
		url:'/21-sun/user/SearchUsersJsonServlet',
		fit:true,
		fitColumns:true,
		idField:'id',
		singleSelect:true,
		...
		{field:'opt',title:'操作',width:20,formatter:function(val,row,idx){	
			...
			"onclick=\"setRole("+row.id+",'"+row.role.role+"')\"/>";
				content+="<input type='button' value='删除账号' onclick=\"del("+row.id+")\"/>";
			return content;
		}}]]
});
	...
	modal:true,
		...
		buttons:[{
			...
			handler:function(){
				...
				var rid=0;
				//var radios=$("input[name='role']");
				$("input[name='role']").each(function(idx,ele){
					if($(ele).attr("checked")){
						rid=$(ele).val();
						return;
					}
				...
				$.ajax({
				url:'/21-sun/user/SetRoleAjaxServlet',
...
function del(nid){
	...
	$.messager.confirm("删除提醒","确认删除吗？",function(r){
		if(r){
			$.ajax({
		url:'/21-sun/user/DeleteUserServlet?uid='+nid,
		//data:{"uid":uid},
		success:function(data){
			$("#sys_user_list").datagrid("reload");
		}
